...
jboolean isCopy;
const char* utfClassName1 = NULL;
if (className != NULL)
   {
   utfClassName1 = (const char *) (*env)->GetStringUTFChars(env, className, &isCopy);
   }
...
#define MAX_RETRY_COUNT 2 
	...
	if (NULL != className) {
		...
		if (validateName && (CLASSNAME_INVALID == vmFuncs->verifyQualifiedName(currentThread, utf8Name, utf8Length, CLASSNAME_VALID_NON_ARRARY, &verifyQualifiedNameReason))) {
...
retry:
	...
	if (J9_ARE_ANY_BITS_SET(*options, J9_FINDCLASS_FLAG_NAME_IS_INVALID) && (NULL == result) && (NULL == currentThread->currentException)) {
		if (className != NULL)
		{
		   printf("DCDCDCDC--> className: %s, LN:273, reason: 0x%08x, classLoaderSuccess: 0x%08x, verifyQualifiedNameReason: 0x%08x\n", utfClassName1, reason, classLoaderSuccess, verifyQualifiedNameReason);
