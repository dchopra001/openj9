...
createUpcallThunk(J9UpcallMetaData *metaData)
{
    ...
    case J9_FFI_UPCALL_SIG_TYPE_STRUCT:
        ...
        else if (returnValueSize < 9) {
            // A single instruction, LG, is used to load data from [R3] into R1
            numInstructions++;
        }
        else if (returnValueSize > 8 && returnValueSize < 17) {
            numInstructions += 2;
        }
        else if (returnValueSize > 16 && returnValueSize < 25) {
            numInstructions += 3;
        }
        else {
    ...
    for (I_32 i = 0; i < lastSigIdx; i++) {
        ...
        case J9_FFI_UPCALL_SIG_TYPE_STRUCT_AGGREGATE_ALL_SP:
            if (argSizeInBytes == 2 * sizeof(float)) {
                ...
                if (gprArgCount < maxGPRArgCount) {
                    // gprsNeededForEntireStruct below represents the number of GPRs we would need hypothetically to load the entire struct in GPRs.
                    // Since this is a pure float structure, the size will always be a multiple of 4. So if we divide by 8, like below, the result
                    // will either be some whole number 'X', or 'X.5'. For `X.5`, adding 0.5 to the sum will always round UP to the nearest whole number.
                    int gprsNeededForEntireStruct = (int)(((float)argSizeInBytes / 8.0F) + 0.5);

                    // We may not have all the GPRs available to store the entire struct. In that case, we store only what's in GPR (the rest of the struct will already be in memory.)
                    int numInstructionsAvailable = maxGPRArgCount - gprArgCount;
                    numInstructions += std::min(gprsNeededForEntireStruct, numInstructionsAvailable);
                    gprArgCount += std::min(gprsNeededForEntireStruct, numInstructionsAvailable);
                }
            }
            break;
        case J9_FFI_UPCALL_SIG_TYPE_STRUCT_AGGREGATE_ALL_DP:
            if (argSizeInBytes == 2 * sizeof(double)) {
                ...
                if (gprArgCount < maxGPRArgCount) {
                    // gprsNeededForEntireStruct below represents the number of GPRs we would need hypothetically to load the entire struct in GPRs.
                    // Since this is a pure double structure, the size will always be a multiple of 4. So if we divide by 8, like below, the result
                    // will either be some whole number 'X'. This operation will always yield a whole number since doubles are expected to be 8 bytes in length.
        ...
        {
            // gprsNeededForEntireStruct below represents the number of GPRs we would need hypothetically to load the entire struct in GPRs.
            // Since this is a general structure, the size wil not have patterns.
            int gprsNeededForEntireStruct = argSizeInBytes % 8 == 0 ? argSizeInBytes / 8 : argSizeInBytes / 8 + 1;
            int numInstructionsAvailable = maxGPRArgCount - gprArgCount;
            numInstructions += std::min(gprsNeededForEntireStruct, numInstructionsAvailable);
            gprArgCount += std::min(gprsNeededForEntireStruct, numInstructionsAvailable);
            break;
    ...
    for (I_32 i = 0; i < lastSigIdx; i++) {
        ...
        case J9_FFI_UPCALL_SIG_TYPE_STRUCT_AGGREGATE_ALL_SP:
            if (argSizeInBytes == 2 * sizeof(float)) {
                ...
                if (gprIndex <= maxGPRIndex) {
                    int totalStackSlotsReq = (int)(((float)argSizeInBytes / 8.0F) + 0.5);
                    int numInstructionsAvailable = maxGPRIndex - gprIndex + 1;
                    int tempArgIndex = currStackSlotIndex;
                        for (int i = 1; i <= std::min(numInstructionsAvailable, totalStackSlotsReq); i++) {
        ...
        case J9_FFI_UPCALL_SIG_TYPE_STRUCT_AGGREGATE_MISC:
            if (gprIndex <= maxGPRIndex) {
                int totalStackSlotsReq = argSizeInBytes % 8 == 0 ? argSizeInBytes / 8 : argSizeInBytes / 8 + 1;
                int numInstructionsAvailable = maxGPRIndex - gprIndex + 1;
                int tempArgIndex = currStackSlotIndex;
                for (int i = 1; i <= std::min(numInstructionsAvailable, totalStackSlotsReq); i++) {
                    thunkMem += STG(thunkMem, gprIndex, 0, R4, argAreaOffsetFromSP + (tempArgIndex * 8));
    ...
    if ((returnType & J9_FFI_UPCALL_SIG_TYPE_MASK) == J9_FFI_UPCALL_SIG_TYPE_STRUCT) {
        ...
        if (returnType == J9_FFI_UPCALL_SIG_TYPE_STRUCT_AGGREGATE_ALL_SP && returnValueSize == 2 * sizeof(float)) {
            ...
            thunkMem += LE(thunkMem, R0, 0, R3, 0);
            thunkMem += LE(thunkMem, R2, 0, R3, 4);
        }
        else if (returnType == J9_FFI_UPCALL_SIG_TYPE_STRUCT_AGGREGATE_ALL_DP && returnValueSize == 2 * sizeof(double)) {
            thunkMem += LD(thunkMem, R0, 0, R3, 0);
            thunkMem += LD(thunkMem, R2, 0, R3, 8);
        }
        else if (returnValueSize > 0 && returnValueSize < 9) {
            thunkMem += LG(thunkMem, R1, 0, R3, 0);
        }
        else if (returnValueSize > 8 && returnValueSize < 17) {
            thunkMem += LG(thunkMem, R1, 0, R3, 0);
            thunkMem += LG(thunkMem, R1, 0, R3, 8);
        }
        else if (returnValueSize > 16 && returnValueSize < 25) {
            thunkMem += LG(thunkMem, R1, 0, R3, 0);
        thunkMem += LG(thunkMem, R1, 0, R3, 8);
        thunkMem += LG(thunkMem, R1, 0, R3, 16);
        }
    ...
    for (I_32 i = 0; i < argIdx; i++) {
        ...
        switch (sigArray[i].type & J9_FFI_UPCALL_SIG_TYPE_MASK) {
            ...
            case J9_FFI_UPCALL_SIG_TYPE_DOUBLE:
                ...
                break;
            case J9_FFI_UPCALL_SIG_TYPE_STRUCT:
                stackSlotCount += argSizeInBytes % 8 == 0 ? argSizeInBytes / 8 : argSizeInBytes / 8 + 1;
                break;
            default:
                Assert_VM_unreachable();
        }
